...
import { Search, Rocket, Briefcase, Image, User, Calendar, Sparkles, Eye, ArrowRight, X, Monitor, Smartphone } from 'lucide-react'
import {
    TEMPLATE_CATALOG,
    TEMPLATE_CATEGORIES,
    searchTemplates,
...
export function TemplateGallery() {
    ...
    return (
        <div className="h-screen overflow-hidden flex" style={{ background: '#0A0E1A' }}>
            {/* Sidebar */}
            <aside className="w-64 flex-shrink-0 border-r flex flex-col" style={{ borderColor: '#1e2540', background: '#0c1024' }}>
                <div className="p-5 border-b" style={{ borderColor: '#1e2540' }}>
                    <h2 className="text-lg font-black text-white mb-1">Plantillas</h2>
                    <p className="text-xs" style={{ color: '#5d7099' }}>Elige y personaliza tu sitio</p>
                </div>
                    ...
                    {TEMPLATE_CATEGORIES.map((cat) => {
                        ...
                        return (
                            <button
                                ...
                                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all"
                                style={{
                                    background: selectedCategory === cat.id ? '#1e2847' : 'transparent',
            ...
            <main className="flex-1 flex flex-col overflow-hidden">
                {/* Search Bar */}
                <header className="flex items-center gap-4 px-6 py-4 border-b" style={{ borderColor: '#1e2540', background: '#0c1024' }}>
                    <div className="flex-1 relative">
                        <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#5d7099' }} />
                        <input
                            type="text"
                            value={search}
                ...
                {fromStrategy && recommendedId && (
                    <div className="mx-6 mt-4 px-4 py-3 rounded-xl flex items-center gap-3" style={{ background: 'linear-gradient(135deg, rgba(255,0,127,0.1), rgba(0,240,255,0.1))', border: '1px solid rgba(0,240,255,0.2)' }}>
                        <Sparkles className="w-5 h-5 flex-shrink-0" style={{ color: '#00F0FF' }} />
                        <p className="text-sm" style={{ color: '#c0d0e8' }}>
                            <strong style={{ color: '#00F0FF' }}>ZentrixOS recomienda</strong> la plantilla
                            <strong className="text-white"> "{getTemplateById(recommendedId)?.name}"</strong> basada en tu análisis estratégico.
                        </p>
                    </div>
                )}
                ...
                <div className="flex-1 overflow-y-auto px-6 py-5">
                    ...
                    {filteredTemplates.length === 0 && (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            ...
                            <p className="text-lg font-semibold text-white/40">No se encontraron plantillas</p>
                            <p className="text-sm mt-1" style={{ color: '#5d7099' }}>Prueba con otro término de búsqueda</p>
                        </div>
                    )}
                </div>
            </main>
...
function TemplateCard({ template, isRecommended, onSelect, onPreview }: {
    ...
    return (
        ...
        >
            {/* Recommended Badge */}
            {isRecommended && (
                <div className="absolute top-3 right-3 z-10 flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold"
                    style={{ background: 'linear-gradient(135deg, #FF007F, #00F0FF)', color: '#fff' }}>
                    <Sparkles className="w-3 h-3" /> Recomendada
                </div>
            )}

            {/* Thumbnail */}
            <div className="relative h-48 overflow-hidden" style={{ background: 'linear-gradient(135deg, #151d38, #0c1024)' }}>
                ...
                />
                <div className="absolute bottom-2 right-2 px-2 py-0.5 rounded-md text-[10px] font-bold"
                ...
                <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 transition-all duration-300 flex items-center justify-center gap-3">
                    <button
                        ...
                        style={{ background: '#1e2847', border: '1px solid #2a3050', color: '#8b9ec7' }}
                    >
            ...
            <div className="p-4">
                ...
                <p className="text-xs leading-relaxed mb-3" style={{ color: '#5d7099' }}>{template.description}</p>
...
function TemplatePreviewModal({ template, onClose, onSelect }: {
    ...
    return (
        <div className="fixed inset-0 z-50 flex" style={{ background: 'rgba(0,0,0,0.85)' }}>
            {/* Sidebar */}
            <div className="w-80 flex-shrink-0 flex flex-col border-r" style={{ background: '#0c1024', borderColor: '#1e2540' }}>
                <div className="p-5 border-b" style={{ borderColor: '#1e2540' }}>
                    <button onClick={onClose} className="flex items-center gap-2 text-sm font-semibold mb-4 transition-all hover:opacity-70" style={{ color: '#5d7099' }}>
                        <X className="w-4 h-4" /> Cerrar Preview
                    </button>
                    <h2 className="text-xl font-black text-white">{template.name}</h2>
                    <p className="text-xs mt-2" style={{ color: '#5d7099' }}>{template.description}</p>
                </div>

                <div className="flex-1 overflow-y-auto p-5 space-y-4">
                    <div>
                        <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: '#5d7099' }}>Secciones Incluidas</p>
                        <div className="space-y-1.5">
                            {template.sections.map((s, i) => (
                                <div key={s.id} className="flex items-center gap-2 text-xs px-3 py-2 rounded-lg"
                                    style={{ background: '#151d38', color: '#8b9ec7' }}>
                                    <span className="w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold"
                                        style={{ background: '#1e2847', color: '#00F0FF' }}>{i + 1}</span>
                                    {s.label}
                                </div>
                            ))}
                        </div>
                    ...
                    <div>
                        <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: '#5d7099' }}>Tags</p>
                        <div className="flex flex-wrap gap-1.5">
                            {template.tags.map((tag) => (
                                <span key={tag} className="text-[10px] px-2 py-1 rounded-md"
                                    style={{ background: '#1e2540', color: '#5d7099' }}>{tag}</span>
                            ))}
                        </div>
                    </div>
                    ...
                    {template.recommendedFor.length > 0 && (
                        <div>
                            <p className="text-[10px] font-bold uppercase tracking-wider mb-2" style={{ color: '#5d7099' }}>Ideal para</p>
                            <div className="space-y-1">
                                {template.recommendedFor.map((r) => (
                                    <p key={r} className="text-xs" style={{ color: '#8b9ec7' }}>• {r}</p>
                                ))}
                            </div>
                        </div>
                    )}
            ...
            <div className="flex-1 flex flex-col">
                {/* Device Toggle */}
                <div className="flex items-center justify-center gap-2 py-3 border-b" style={{ borderColor: '#1e2540', background: '#0c1024' }}>
                    <button
                ...
                <div className="flex-1 overflow-y-auto flex items-start justify-center p-8">
                    <div style={{ width: device === 'mobile' ? '390px' : '100%', maxWidth: '1200px', borderColor: '#1e2540' }}
                    ...
                    >
                        ...
                        <div className="px-6 py-4" style={{ background: '#111827' }}>
                            <p className="text-[10px] font-bold uppercase tracking-widest mb-3" style={{ color: '#00F0FF' }}>
                                Secciones Editables
                            </p>
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                {template.sections.map((section, i) => (
                                    <div key={section.id} className="flex items-center gap-2 text-xs px-3 py-2 rounded-lg"
                                        style={{ background: '#151d38', color: '#8b9ec7' }}>
