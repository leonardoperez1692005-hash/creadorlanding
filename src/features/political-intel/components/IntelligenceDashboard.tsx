        ...
        <div style={{
            ...
            <div style={{
                ...
                <div style={{
                    ...
                    <StatBadge label="Perfiles analizados" value={String(snapshot.meta.successfulScrapes)} color="#00c8ff" />
                    <StatBadge label="Fallidos" value={String(snapshot.failedHandles.length)} color="#F87171" />
                    <StatBadge label="Costo Bright Data" value={snapshot.meta.estimatedCost} color="#34D399" />
                    <StatBadge label="Generado" value={new Date(report.generatedAt).toLocaleDateString('es-AR')} color="#A78BFA" />
                </div>
            </div>
                ...
                <div style={{
                    ...
                    {snapshot.profiles.map(profile => {
                        ...
                        const analysis = report.comparativeAnalysis.find(
                            c => c.handle === profile.handle || c.handle === `@${profile.handle.replace('@', '')}`
                        )
                        if (!m) return null
                        return (
                            <ProfileCard
            ...
            {snapshot.failedHandles.length > 0 && (
                <div style={{
                    ...
                    Handles fallidos: {snapshot.failedHandles.map(f => `@${f.handle} (${f.error})`).join(', ')}
                </div>
            )}
