...
type SortKey = 'followers' | 'following' | 'followerToFollowingRatio' | 'tweetsPerDay' | 'audienceEfficiency'
...
export function RankingTable({ metrics }: RankingTableProps) {
    ...
    const headerStyle = (key: SortKey): React.CSSProperties => ({
        padding: '0.6rem 0.75rem', textAlign: 'right' as const, fontSize: '0.75rem',
        color: sortKey === key ? '#00c8ff' : '#6B7280', cursor: 'pointer',
        fontWeight: sortKey === key ? 700 : 500, whiteSpace: 'nowrap' as const,
        userSelect: 'none' as const, borderBottom: '1px solid #1e2540',
    })

    const cellStyle: React.CSSProperties = {
        padding: '0.6rem 0.75rem', textAlign: 'right', fontSize: '0.8rem',
        color: '#8b9ec7', borderBottom: '1px solid rgba(30,37,64,0.5)',
    }
    ...
    return (
        <div style={{
            ...
            <div style={{
                padding: '1rem 1.25rem 0.5rem', color: '#fff', fontWeight: 700,
                fontSize: '1rem', borderBottom: '1px solid #1e2540',
            }}>
                Ranking Comparativo
            </div>
            <div style={{ overflowX: 'auto' }}>
                <table style={{
                    ...
                    <thead>
                        <tr>
                            <th style={{
                                ...
                                color: '#6B7280', borderBottom: '1px solid #1e2540', width: 40,
                            }}>#</th>
                            <th style={{
                                padding: '0.6rem 0.75rem', textAlign: 'left', fontSize: '0.75rem',
                                color: '#6B7280', borderBottom: '1px solid #1e2540',
                            }}>Politico</th>
                            <th style={{
                                padding: '0.6rem 0.75rem', textAlign: 'left', fontSize: '0.75rem',
                                color: '#6B7280', borderBottom: '1px solid #1e2540',
                            ...
                            {columns.map(col => (
                                <th key={col.key} style={headerStyle(col.key)} onClick={() => handleSort(col.key)}>
                    ...
                    <tbody>
                        {sorted.map((m, i) => (
                            <tr key={m.handle} style={{
                                background: i % 2 === 0 ? 'transparent' : 'rgba(255,255,255,0.02)',
                            }}>
                                <td style={{ ...cellStyle, textAlign: 'left', color: '#6B7280' }}>{i + 1}</td>
                                <td style={{ ...cellStyle, textAlign: 'left', color: '#fff', fontWeight: 600 }}>
                                    {m.displayName}
                                    <span style={{ color: '#00c8ff', fontSize: '0.7rem', marginLeft: '0.4rem' }}>
                                        {m.handle}
                                    </span>
                                </td>
