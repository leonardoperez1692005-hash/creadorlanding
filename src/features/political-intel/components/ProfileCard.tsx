...
export function ProfileCard({ profile, metrics, strengths = [], weaknesses = [] }: ProfileCardProps) {
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                {profile.profileImageUrl ? (
                    <img
                        ...
                        style={{
                            ...
                            border: '2px solid #1e2540', objectFit: 'cover',
                        }}
                    />
                ) : (
                    <div style={{
                        width: 48, height: 48, borderRadius: '50%',
                        background: 'rgba(0,200,255,0.1)', border: '2px solid #1e2540',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        color: '#00c8ff', fontSize: '1.2rem', fontWeight: 700,
                    }}>
                ...
                <div>
                    ...
                    <div style={{ color: '#00c8ff', fontSize: '0.8rem' }}>
                        {profile.handle}
                    </div>
            ...
            {(strengths.length > 0 || weaknesses.length > 0) && (
                <div style={{ display: 'flex', gap: '0.75rem', fontSize: '0.75rem' }}>
                    {strengths.length > 0 && (
                        <div style={{ flex: 1 }}>
                            <div style={{ color: '#34D399', fontWeight: 600, marginBottom: '0.3rem' }}>Fortalezas</div>
                            {strengths.map((s, i) => (
                                <div key={i} style={{ color: '#8b9ec7', marginBottom: '0.15rem' }}>+ {s}</div>
                            ))}
                        </div>
                    )}
                    {weaknesses.length > 0 && (
                        <div style={{ flex: 1 }}>
                            <div style={{ color: '#F87171', fontWeight: 600, marginBottom: '0.3rem' }}>Debilidades</div>
                            {weaknesses.map((w, i) => (
                                <div key={i} style={{ color: '#8b9ec7', marginBottom: '0.15rem' }}>- {w}</div>
                            ))}
                        </div>
                    )}
                </div>
...
function MetricBox({ label, value, color }: { label: string; value: string; color?: string }) {
    return (
        <div style={{
            background: 'rgba(255,255,255,0.03)', borderRadius: '8px', padding: '0.5rem',
            textAlign: 'center',
        }}>
            <div style={{ color: '#6B7280', fontSize: '0.65rem', marginBottom: '0.15rem' }}>{label}</div>
            <div style={{ color: color ?? '#fff', fontSize: '0.85rem', fontWeight: 700 }}>{value}</div>
        </div>
    )
}
