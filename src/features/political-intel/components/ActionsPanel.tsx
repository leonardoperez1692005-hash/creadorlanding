...
export function ActionsPanel({ actions, comparativeAnalysis, executiveSummary }: ActionsPanelProps) {
    ...
    function handleCreateStrategy() {
        ...
        localStorage.setItem('zentrix_intel_brief', JSON.stringify(briefData))
    ...
    async function handleCreateLanding() {
        ...
        const strategyForLanding = {
            ...
            landingBlueprint: {
                recommendedType: 'VSL',
                sections: [
                    { name: 'Hero', purpose: 'Captar atencion', suggestedContent: executiveSummary.substring(0, 200) },
                    { name: 'Beneficios', purpose: 'Mostrar valor', suggestedContent: topAction?.action ?? '' },
                ],
            },
        ...
        localStorage.setItem('zentrix_strategy_for_landing', JSON.stringify(strategyForLanding))
        localStorage.setItem('zentrix_strategy_brief', JSON.stringify(briefData))

        // Navigate to strategy to use generateLandingContentAction
        localStorage.setItem('zentrix_intel_landing_request', JSON.stringify({
    ...
    return (
        <div style={{
            ...
            <div style={{
                ...
                marginBottom: '1rem', paddingBottom: '0.75rem', borderBottom: '1px solid #1e2540',
            }}>
                <h3 style={{
                    color: '#FF007F', fontSize: '0.95rem', fontWeight: 700,
                ...
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <button
                        ...
                        style={{
                            ...
                            color: '#A78BFA', cursor: loadingStrategy ? 'not-allowed' : 'pointer',
                        }}
                    >
                        {loadingStrategy
                    ...
                    <button
                        ...
                        style={{
                            ...
                            color: '#00c8ff', cursor: loadingLanding ? 'not-allowed' : 'pointer',
                        }}
                    >
                        {loadingLanding
                            ? <Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} />
                ...
                {actions.map((action, i) => {
                    ...
                    return (
                        <div key={i} style={{
                            ...
                            <div style={{ color: '#8b9ec7', fontSize: '0.78rem', marginBottom: '0.2rem' }}>
                                {action.rationale}
                            </div>
