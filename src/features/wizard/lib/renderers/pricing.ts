...
export const pricingRenderer: SectionRenderer = (c, t, _ctx) => {
  ...
  const cards = items.map((plan: any, i: number) => {
    const features = Array.isArray(plan.features) ? plan.features : []
    const featList = features.map((f: string) => `<li style="padding:8px 0;border-bottom:1px solid var(--muted);opacity:.8">✓ ${esc(f)}</li>`).join('')
