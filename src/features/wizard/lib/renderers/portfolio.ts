...
export const portfolioRenderer: SectionRenderer = (c, _t, _ctx) => {
  ...
  const cards = items.map((item: any, i: number) => `
    <div class="benefit-card reveal" style="padding:0;overflow:hidden" ${delay(i)}>
      ${item.image ? `<img src="${esc(item.image)}" alt="${esc(item.title || '')}" loading="lazy" style="width:100%;height:220px;object-fit:cover">` : '<div style="width:100%;height:220px;background:var(--muted);display:flex;align-items:center;justify-content:center;opacity:.2;font-weight:900;font-size:1.5rem">FOTO</div>'}
