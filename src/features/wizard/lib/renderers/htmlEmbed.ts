...
export const htmlEmbedRenderer: SectionRenderer = (c, _t, _ctx) => {
  ...
  ${title}
  <div class="reveal" style="max-width:${esc(maxW)};margin:0 auto">${sanitizeHtml(c.html_code || '<p style="text-align:center;opacity:.4">Pega tu código HTML aquí</p>', true)}</div>
</div></section>`
}
