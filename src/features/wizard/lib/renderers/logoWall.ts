    ...
    const logos = items
        .map((item: RendererItem, i: number) => {
            if (item.logo || item.image) {
                return `<div class="reveal" style="display:flex;align-items:center;justify-content:center;padding:20px;opacity:.6;transition:opacity .3s" ${delay(i)}><img src="${esc(item.logo || item.image)}" alt="${esc(item.name || '')}" loading="lazy" width="120" height="40" style="max-height:40px;max-width:120px;filter:grayscale(1);transition:filter .3s"></div>`
            }
