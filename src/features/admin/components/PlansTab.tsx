...
import {
    ...
    StatusBadge, inputClass, inputStyle, focusStyle, blurStyle,
} from './AdminShared'
import {
    fetchAdminPlansAction, createPlanAction, updatePlanAction, deletePlanAction,
    type PlanInput,
} from '../actions'
...
export function PlansTab({ currentUserRole, showToast }: PlansTabProps) {
    ...
    const handleCreate = async (data: PlanInput) => {
        ...
        if (result.success) { showToast('Plan creado'); setModal(null); load() }
        else showToast(result.error, 'error')
    }

    const handleUpdate = async (id: string, data: Partial<PlanInput>) => {
        const result = await updatePlanAction(id, data)
        if (result.success) { showToast('Plan actualizado'); setModal(null); load() }
        else showToast(result.error, 'error')
    }
    ...
    const handleDelete = (p: PlanRecord) => {
        setConfirm({
            ...
            onConfirm: async () => {
                ...
                if (result.success) { showToast('Plan eliminado'); setConfirm(null); load() }
                else showToast(result.error, 'error')
            },
        })
    }
        ...
        <div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {filtered.map((p) => {
                        const activeCount = p.memberships?.filter((m) => m.status === 'active').length ?? 0
                        return (
                            <div key={p.id} className="p-5 rounded-2xl transition-all hover:scale-[1.01]"
                                ...
                                <div className="text-3xl font-black mb-4" style={{ color: 'var(--cyan)' }}>
                                    ${p.price}
                                    <span className="text-base font-normal ml-1" style={{ color: 'var(--text-muted)' }}>/{p.currency}</span>
                                </div>
                                <div className="space-y-1.5 mb-4 text-xs" style={{ color: 'var(--text-secondary)' }}>
                                    <div>📁 Hasta <strong>{p.max_projects}</strong> proyectos</div>
                                    <div>👥 Hasta <strong>{p.max_leads}</strong> leads</div>
                                    <div>🎫 <strong>{activeCount}</strong> membresías activas</div>
                                </div>
                                {currentUserRole === 'superadmin' && (
            ...
            {(modal?.type === 'create' || modal?.type === 'edit') && (
                <PlanFormModal
                    ...
                    initial={modal.data}
                    onSubmit={modal.type === 'create' ? handleCreate : (d) => handleUpdate(modal.data!.id, d)}
            ...
            <form onSubmit={(e) => { e.preventDefault(); onSubmit(form) }}>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <FormField label="Moneda">
                        <select value={form.currency} onChange={(e) => set('currency', e.target.value as 'USD' | 'EUR' | 'ARS' | 'MXN')}
                            className={inputClass} style={inputStyle} onFocus={focusStyle} onBlur={blurStyle}>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    <FormField label="Máx. Leads">
                        <input type="number" min="0" value={form.max_leads}
