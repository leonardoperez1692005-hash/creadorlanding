...
import { Edit3, Ban, RefreshCcw, Trash2, Plus, Loader2, Eye, Key } from 'lucide-react'
import {
    AdminModal, ConfirmModal, FormField, SearchBar, Toast,
    AdminTable, Th, Td, Tr, StatusBadge, RoleBadge,
    inputClass, inputStyle, focusStyle, blurStyle,
} from './AdminShared'
import {
    fetchAdminUsersAction, updateUserAction, deleteUserAction, createUserAction,
} from '../actions'
...
export function UsersTab({ currentUserRole, showToast }: UsersTabProps) {
    ...
    const [modal, setModal] = useState<{ type: 'create' | 'edit' | 'detail'; data?: UserRecord } | null>(null)
    ...
    const handleSuspend = async (u: UserRecord) => {
        ...
        if (result.success) { showToast('Usuario suspendido'); load() }
        else showToast(result.error, 'error')
    }

    const handleReactivate = async (u: UserRecord) => {
        const result = await updateUserAction(u.id, { status: 'active' })
        if (result.success) { showToast('Usuario reactivado'); load() }
        else showToast(result.error, 'error')
    }
    ...
    const handleDelete = (u: UserRecord) => {
        setConfirm({
            ...
            onConfirm: async () => {
                ...
                if (result.success) { showToast('Usuario eliminado'); setConfirm(null); load() }
                else showToast(result.error, 'error')
            },
        })
    }
    ...
    return (
        <div>
            ...
            {loading ? (
                ...
                <AdminTable>
                    <thead>
                        <tr><Th>Email</Th><Th>Nombre</Th><Th>Rol</Th><Th>Status</Th><Th>Plan activo</Th><Th>Registro</Th><Th>Acciones</Th></tr>
                    </thead>
                    <tbody>
                        {filtered.map((u) => {
                            const activePlan = u.memberships?.find((m) => m.status === 'active')?.plan?.name ?? '—'
                            return (
                                <Tr key={u.id}>
                                    <Td><span className="text-white">{u.email}</span></Td>
                                    <Td><span style={{ color: 'var(--text-secondary)' }}>{u.name ?? '—'}</span></Td>
                                    <Td><RoleBadge role={u.role} /></Td>
                                    <Td><StatusBadge status={u.status} /></Td>
                                    <Td><span className="text-sm" style={{ color: 'var(--cyan)' }}>{activePlan}</span></Td>
                                    <Td><span className="text-xs" style={{ color: 'var(--text-muted)' }}>{new Date(u.created_at).toLocaleDateString('es-AR')}</span></Td>
                                    <Td>
                                        <div className="flex gap-1.5">
                                            <button onClick={() => setModal({ type: 'detail', data: u })}
                                                className="p-1.5 rounded hover:bg-white/10 transition-all" title="Ver detalle"
                                            ...
                                            {u.status === 'active' && u.role !== 'superadmin' && (
                                                <button onClick={() => handleSuspend(u)}
                                                    className="p-1.5 rounded hover:bg-amber-500/20 transition-all" title="Suspender"
                                                    style={{ color: 'var(--text-muted)' }}>
                                            ...
                                            {u.status === 'suspended' && (
                                                <button onClick={() => handleReactivate(u)}
                                                    className="p-1.5 rounded hover:bg-emerald-500/20 transition-all" title="Reactivar"
                                                    style={{ color: 'var(--text-muted)' }}>
            ...
            {modal?.type === 'detail' && modal.data && (
                <UserDetailModal userId={modal.data.id} onClose={() => setModal(null)} showToast={showToast} />
            )}
...
function CreateUserModal({ onClose, onCreated, showToast }: {
    ...
    const [form, setForm] = useState({ email: '', password: '', name: '', role: 'user' as 'user' | 'admin' | 'superadmin' })
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setSaving(false)
        if (result.success) { showToast('Usuario creado'); onCreated() }
        else showToast(result.error, 'error')
    }
...
// Edit User Modal
// ============================================================
function EditUserModal({ user, currentUserRole, onClose, onSaved, showToast }: {
    user: UserRecord
    currentUserRole: string
    onClose: () => void
    onSaved: () => void
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        setSaving(true)
        const input: { name?: string; email?: string; role?: typeof form.role; status?: typeof form.status } = {}
        ...
        setSaving(false)
        if (result.success) { showToast('Usuario actualizado'); onSaved() }
        else showToast(result.error, 'error')
    }
