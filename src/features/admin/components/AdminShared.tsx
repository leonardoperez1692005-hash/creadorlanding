...
export function StatusBadge({ status }: { status: string }) {
    return (
        <span
            className={`text-xs px-2 py-0.5 rounded-full border ${STATUS_STYLES[status] ?? STATUS_STYLES.inactive}`}
        >
            {status}
        </span>
    )
...
export function RoleBadge({ role }: { role: string }) {
    return (
        <span
            className={`text-xs px-2 py-0.5 rounded-full border ${ROLE_STYLES[role] ?? ROLE_STYLES.user}`}
        >
            {role}
        </span>
    )
...
export function AdminModal({ title, children, onClose, maxWidth = 'max-w-lg' }: AdminModalProps) {
    return (
        ...
        >
            {/* eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions */}
            <div
                className={`w-full ${maxWidth} max-h-[85vh] overflow-auto rounded-2xl shadow-2xl`}
...
export function Toast({ message, type }: ToastProps) {
    return (
        <div
            className={`fixed top-5 right-5 z-50 px-4 py-3 rounded-xl text-sm shadow-2xl border ${
                type === 'error'
