...
import {
    AdminModal, ConfirmModal, SectionHeader, EmptyState,
    AdminTable, Th, Td, Tr, StatusBadge, RoleBadge,
} from './AdminShared'
import {
    fetchUserDetailAction, createLicenseAction, updateLicenseAction,
    revokeLicenseAction, deleteLicenseAction,
} from '../actions'
...
export function UserDetailModal({ userId, onClose, showToast }: UserDetailModalProps) {
    ...
    const handleCreateLicense = async () => {
        ...
        if (result.success) { showToast('Licencia creada'); load() }
    ...
    const handleReactivateLicense = async (licenseId: string) => {
        ...
        if (result.success) { showToast('Licencia reactivada'); load() }
        else showToast(result.error, 'error')
    }
                ...
                <div>
                    ...
                    <div className="flex items-center gap-4 mb-6 pb-5" style={{ borderBottom: '1px solid var(--border)' }}>
                        <div className="w-12 h-12 rounded-full flex items-center justify-center text-lg font-bold"
                            style={{ background: 'linear-gradient(135deg, var(--pink), var(--cyan))', color: '#000' }}>
                            {(data.profile.name?.[0] ?? data.email[0]).toUpperCase()}
                        </div>
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                                <span className="text-white font-semibold truncate">{data.profile.name ?? '—'}</span>
                                <RoleBadge role={data.profile.role} />
                                <StatusBadge status={data.profile.status} />
                            </div>
                            <p className="text-sm truncate" style={{ color: 'var(--text-muted)' }}>{data.email}</p>
                            <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                                Registrado: {new Date(data.profile.created_at).toLocaleDateString('es-AR')}
                            </p>
                        </div>
                    </div>
                    ...
                    {data.licenses.length === 0 ? (
                        ...
                        <AdminTable className="mb-2">
                            <thead>
                                <tr><Th>Key</Th><Th>Dominio</Th><Th>Status</Th><Th>Expiración</Th><Th>Acciones</Th></tr>
                            </thead>
                            <tbody>
                                {data.licenses.map((l) => (
                                    <Tr key={l.id}>
                                        <Td><span className="font-mono text-xs" style={{ color: 'var(--cyan)' }}>{l.key}</span></Td>
                                        ...
                                        <Td>
                                            <div className="flex gap-1">
                                                {l.status === 'active' && (
                                                    <button onClick={() => handleRevokeLicense(l.id, l.key)}
                                                        className="p-1 rounded hover:bg-amber-500/20 transition-all" title="Revocar"
                                                        style={{ color: 'var(--text-muted)' }}>
                                                        <ShieldOff className="w-3 h-3" />
                                                    </button>
                                                )}
                                                {l.status === 'revoked' && (
                                                    <button onClick={() => handleReactivateLicense(l.id)}
                                                        className="p-1 rounded hover:bg-emerald-500/20 transition-all" title="Reactivar"
                                                        style={{ color: 'var(--text-muted)' }}>
                                                        <Key className="w-3 h-3" />
                                                    </button>
                                                )}
                                                <button onClick={() => handleDeleteLicense(l.id, l.key)}
                                                    className="p-1 rounded hover:bg-red-500/20 transition-all" title="Eliminar"
                                                    style={{ color: 'var(--text-muted)' }}>
                    ...
                    {data.projects.length === 0 ? (
                        ...
                        <AdminTable className="mb-2">
                            <thead>
                                <tr><Th>Nombre</Th><Th>Tipo</Th><Th>Tema</Th><Th>Fecha</Th><Th>Link</Th></tr>
                            </thead>
                            <tbody>
                                {data.projects.map((p) => (
                                    <Tr key={p.id}>
                                        <Td><span className="text-white">{p.name}</span></Td>
                                        <Td><span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{p.structure_type}</span></Td>
                                        <Td><span className="text-xs" style={{ color: 'var(--text-secondary)' }}>{p.visual_model}</span></Td>
                                        <Td><span className="text-xs" style={{ color: 'var(--text-muted)' }}>{new Date(p.created_at).toLocaleDateString('es-AR')}</span></Td>
                                        <Td>
                    ...
                    {data.memberships.length === 0 ? (
                        ...
                        <AdminTable>
                            <thead>
                                <tr><Th>Plan</Th><Th>Status</Th><Th>Inicio</Th><Th>Expiración</Th></tr>
                            </thead>
                            <tbody>
