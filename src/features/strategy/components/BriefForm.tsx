...
import { Shield, Globe, Target, Zap, Plus, X, Loader2, ChevronDown, Radar, Pencil } from 'lucide-react'
...
export function BriefForm({ onStartAnalysis }: BriefFormProps) {
    ...
    const [competitors, setCompetitors] = useState<string[]>(briefData.competitors ? briefData.competitors.split(',').filter(Boolean) : [''])
    ...
    useEffect(() => {
        ...
        if (fromIntelligence) {
            try {
                const raw = localStorage.getItem('zentrix_intel_brief')
                if (raw) {
                    ...
                    const comps = (data.competitors || '').split(/[\n,]/).map((s: string) => s.trim()).filter(Boolean)
                    if (comps.length > 0) setCompetitors(comps)
                    setSource('intelligence')
                    localStorage.removeItem('zentrix_intel_brief')
                }
    ...
    const updateCompetitor = (i: number, v: string) => setCompetitors(competitors.map((c, idx) => idx === i ? v : c))

    const isValid = briefData.sector?.trim() && briefData.brandName?.trim() && competitors.some((c) => c.trim())

    const inputClass = "w-full px-8 py-6 rounded-[24px] text-lg text-white bg-black/40 border border-white/10 outline-none transition-all focus:border-cyan-500/50 focus:bg-black/60"

    const Section = ({ title, icon: Icon, badge, children }: { title: string; icon: React.ElementType; badge?: string; children: React.ReactNode }) => (
        <div className="rounded-[32px] p-10 md:p-14 mb-16 transition-all hover:shadow-[0_20px_80px_-20px_rgba(0,0,0,0.8)]"
            style={{ background: 'var(--bg-card)', border: '1px solid var(--border)' }}>
            <div className="flex items-center gap-6 mb-12">
                <div className="p-4 rounded-2xl flex items-center justify-center shadow-lg" style={{ background: 'rgba(0,200,255,0.05)', border: '1px solid rgba(0,200,255,0.2)' }}>
                    <Icon className="w-8 h-8" style={{ color: 'var(--cyan)' }} />
                </div>
                <h3 className="text-xl font-black tracking-[0.3em] text-white uppercase">{title}</h3>
                {badge && (
                    <span className="text-[10px] font-bold tracking-wider uppercase px-3 py-1 rounded-full"
                        style={{
                            background: source === 'intelligence' ? 'rgba(0,200,255,0.1)' : 'rgba(124,58,237,0.1)',
    ...
    const sourceBadge = source === 'intelligence' ? 'Desde Intelligence' : source === 'branding' ? 'Desde Branding' : undefined
    ...
    return (
        <div className="max-w-5xl mx-auto pb-64 px-6">
            ...
            <div className="flex flex-col items-center justify-center text-center pt-40 pb-32">
                <div className="inline-flex items-center gap-4 px-8 py-3 rounded-full mb-16 shadow-2xl"
                    ...
                    <span className="text-xs font-black tracking-[0.4em] uppercase" style={{ color: 'var(--cyan)' }}>
                        ZENTRIX OS v2 · Powered by Bright Data
                    </span>
                </div>
                ...
                <p className="text-xl md:text-3xl max-w-3xl mx-auto leading-relaxed font-medium opacity-80" style={{ color: 'var(--text-secondary)' }}>
                    {fromIntelligence
                ...
                {/* Source indicator */}
                {source !== 'manual' && (
                    <div className="mt-8 inline-flex items-center gap-3 px-5 py-2 rounded-full text-xs font-bold tracking-wider"
                        style={{
                            background: source === 'intelligence' ? 'rgba(0,200,255,0.06)' : 'rgba(124,58,237,0.06)',
                            border: `1px solid ${source === 'intelligence' ? 'rgba(0,200,255,0.15)' : 'rgba(124,58,237,0.15)'}`,
                            color: source === 'intelligence' ? '#00c8ff' : '#A78BFA',
                        }}>
                        {source === 'intelligence' ? <Radar className="w-3.5 h-3.5" /> : <Shield className="w-3.5 h-3.5" />}
                        {source === 'intelligence' ? 'Datos desde Intelligence' : 'Datos desde Branding'}
            ...
            <form onSubmit={handleSubmit} className="space-y-16">
                <Section title="Identidad de Marca" icon={Shield} badge={sourceBadge}>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        <div className="space-y-4">
                            <label className="text-xs font-black tracking-[0.2em] text-white/40 uppercase block ml-1">
                                Tu Marca
                            </label>
                            <input
                    ...
                    <div className="space-y-4">
                        ...
                        <textarea
                            ...
                            placeholder={loadingProfile ? "Cargando..." : "Tus valores estratégicos..."}
                            rows={3}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                        <div className="space-y-4">
                            ...
                            <input
                                ...
                                placeholder={loadingProfile ? "Cargando..." : "Ej: Aumentar ventas online"}
                                className={inputClass}
                            />
                        </div>
                        <div className="space-y-4">
                            ...
                            <div className="relative">
                                ...
                                >
                                    {COUNTRIES.map((c) => <option key={c.code} value={c.code} className="bg-[#0A0E1A]">{c.name}</option>)}
                                </select>
                ...
                <Section title="Radar de Competencia" icon={Globe}>
                    <p className="text-lg font-medium opacity-60 leading-relaxed mb-8">
                        Identifica hasta 5 competidores directos o referentes del sector para iniciar la extracción táctica.
                    </p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        {competitors.length < 5 && (
                            <button type="button" onClick={addCompetitor}
                                ...
                                style={{ color: 'var(--cyan)', border: '2px dashed rgba(0,200,255,0.1)', background: 'rgba(0,200,255,0.02)' }}>
                                <Plus className="w-5 h-5 group-hover:scale-110 transition-transform" /> Expandir Radar
                            </button>
                        )}
                    </div>
                </Section>
                ...
                <div className="pt-20">
                    <button type="submit" disabled={!isValid || submitting}
                        ...
                        {submitting ? (
                            <><Loader2 className="w-8 h-8 animate-spin" /> PROCESANDO MATRIZ TÁCTICA...</>
                        ) : (
                    ...
                    <p className="text-xs text-center mt-10 uppercase tracking-[0.5em] font-black opacity-30">
                        Zentrix Intelligence Matrix Core v2.4
                    </p>
                </div>
            </form>
        </div>
