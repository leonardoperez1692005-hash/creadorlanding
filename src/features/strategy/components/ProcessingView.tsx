...
const PHASES = [
    ...
    { key: 'ZentrixOS', icon: Brain, color: 'var(--pink)', label: 'Strategy Synthesis' },
]
        ...
        <div className="max-w-lg mx-auto text-center py-16">
            ...
            <div className="relative w-32 h-32 mx-auto mb-10">
                ...
                <div className="absolute inset-3 rounded-full border-2 border-transparent animate-spin"
                    style={{ borderTopColor: 'var(--pink)', animationDirection: 'reverse', animationDuration: '1.5s' }} />
            ...
            <p className="text-sm mb-8 animate-pulse" style={{ color: 'var(--text-secondary)' }}>{status || 'Procesando datos...'}</p>
            ...
            <div className="space-y-3">
                {PHASES.map((phase, i) => {
                    ...
                    return (
                        <div key={phase.key} className="flex items-center gap-3 px-4 py-3 rounded-xl transition-all"
                            style={{
                            ...
                            <div className="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                                ...
                                {done ? (
                                    ...
                                    <Loader2 className="w-4 h-4 animate-spin" style={{ color: phase.color }} />
                                ) : (
                                    <Icon className="w-4 h-4 opacity-30 text-white" />
                                )}
                            </div>
                            <div className="text-left">
                                <div className="text-xs font-semibold" style={{ color: done || active ? phase.color : 'var(--text-muted)' }}>
                                    {phase.key}
                                </div>
                                <div className="text-xs" style={{ color: 'var(--text-muted)' }}>{phase.label}</div>
                            </div>
                            {done && <span className="ml-auto text-xs font-code" style={{ color: '#10b981' }}>✓ OK</span>}
                            {active && <span className="ml-auto text-xs font-code animate-pulse" style={{ color: phase.color }}>ACTIVO</span>}
                        </div>
                    )
                })}
            </div>
