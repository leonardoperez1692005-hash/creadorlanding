...
export function TacticalOverlay({ type, salesAngle, data, onClose }: TacticalOverlayProps) {
    return (
        ...
        >
            {/* eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions */}
            <div
                className="w-full max-w-2xl max-h-[85vh] overflow-auto rounded-2xl shadow-2xl"
            ...
            >
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
                                Ángulo: &ldquo;{salesAngle.substring(0, 60)}&hellip;&rdquo;
                            </p>
                        </div>
                    </div>
                    <button
                ...
                <div className="p-5">
                    ...
                    {type === 'content' && <ContentView data={data as unknown as ContentData} />}
                </div>
            </div>
        </div>
    )
            ...
            {data.objections?.map((obj, i) => (
                ...
                >
                    <div className="px-4 py-3" style={{ background: 'rgba(239,68,68,0.08)' }}>
                        <span className="text-xs font-code text-red-400 block mb-1">
                            OBJECIÓN #{i + 1}
                        </span>
    ...
    return (
        <div className="space-y-5">
            ...
            <div
                ...
                style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(255,0,127,0.2)' }}
            >
                <div className="flex items-center gap-2 mb-3">
                    <span className="text-base">🎵</span>
                    <span className="text-sm font-bold text-white">TikTok</span>
                    <span className="text-xs font-code" style={{ color: 'var(--pink)' }}>
                        GUIÓN DISRUPTOR
                    </span>
                </div>
                <div className="space-y-3 text-base">
                    <div>
                        <span
                            className="text-xs font-code block mb-1"
                            style={{ color: 'var(--text-muted)' }}
                        >
                            PRIMER GANCHO
                        </span>
                        <p className="text-white font-semibold">
                            &ldquo;{data.tiktok?.hook}&rdquo;
                        </p>
                    </div>
                    <div>
                        <span
            ...
            <div
                className="rounded-xl p-4"
                style={{ background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(0,119,181,0.3)' }}
            >
                <div className="flex items-center gap-2 mb-3">
                    <span className="text-base">💼</span>
                    <span className="text-sm font-bold text-white">LinkedIn</span>
                    <span className="text-xs font-code" style={{ color: '#0077b5' }}>
                        POST DE AUTORIDAD
                    </span>
                </div>
                <div className="space-y-3 text-base">
                        ...
                        <div className="flex flex-wrap gap-2">
                            {data.linkedin.hashtags.map((h, i) => (
                                <span
                                    ...
                                    style={{ background: 'rgba(0,119,181,0.15)', color: '#60a5fa' }}
                                >
                                    #{h.replace(/^#/, '')}
                                </span>
                            ))}
